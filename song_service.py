...
class SaavnService:
    ...
    def search_songs_by_mood(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        mood_queries = {
            "Happy": ["bollywood dance songs", "happy hindi songs", "punjabi upbeat songs", "feel good bollywood", "dil chahta hai", "kal ho naa ho"],
            "Sad": ["sad bollywood songs", "arijit singh sad songs", "heartbreak hindi songs", "tum hi ho", "ae dil hai mushkil", "kabira"],
            "Angry": ["rock hindi songs", "powerful bollywood songs", "intense indian music", "zinda", "rang de basanti", "dangal songs"],
            "Anxious": ["calming hindi songs", "peaceful bollywood music", "soothing indian songs", "raag yaman", "classical indian music", "meditation songs"],
            "Relaxed": ["soft hindi songs", "romantic bollywood songs", "chill indian music", "tere naam", "kumar sanu songs", "peaceful hindi"],
            "Neutral": ["popular bollywood songs", "trending hindi songs", "top indian hits", "arijit singh", "atif aslam", "rahat fateh ali khan"]
        }
    ...
    def get_song_details(self, song_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if data.get("success") and "data" in data:
                # Handle case where data["data"] might be a list
                song_data = data["data"]
                if isinstance(song_data, list) and len(song_data) > 0:
                    return song_data[0]  # Return the first song
                elif isinstance(song_data, dict):
                    return song_data
                else:
                    logger.error(f"Unexpected data structure: {type(song_data)}")
                    return {}
            else:
